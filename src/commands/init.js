...
async function generateFiles(targetDir, modelMap, stateManager) {
  section('生成文件');
...
async function copySkills(targetDir, stateManager) {
  ...
  for (const skill of skills) {
    // 如果 stateManager 为 false，跳过 state-manager skill
...
export async function init(args) {
  ...
  const targetDir = getTargetDir();
    ...
    if (opts.model) {
      meta(`模型：${opts.model}`);
      await generateFiles(targetDir, { _default: opts.model }, opts.stateManager);
    } else if (opts.yes) {
      const m = allModels?.[0] || DEFAULT_MODEL;
      meta(`模型：${m}`);
      await generateFiles(targetDir, { _default: m }, opts.stateManager);
    } else {
      ...
      if (unified) {
        ...
        await generateFiles(targetDir, modelMap, opts.stateManager);
      }
    }
    ...
    section('Skills');
    await copySkills(targetDir, opts.stateManager);
