...
1. **任务开始时**：立即执行以下命令保存任务状态：
   ```bash
   bash bin/task-state save <task-id> --agent fullstack-developer --status in_progress --summary "<任务摘要>" --user-request "<用户原始需求>"
   ```
...
{{#stateManager}}
- **中断恢复**：启动时用 `bash bin/task-state list --agent fullstack-developer` 检查未完成任务，用 `bash bin/task-state load <task-id>` 读取上下文继续执行，进度更新时用 `bash bin/task-state save` 保存状态
{{/stateManager}}
...
**绝对禁止：**
- 创建新的项目根目录（如 `mkdir my-project`）
- 把文件放到当前目录之外

**必须遵守：**
- 所有文件直接在当前工作目录下创建
- 如果需要子目录（如 `src/`、`public/`），直接在当前目录下创建
- 项目入口文件（如 `package.json`、`index.html`）放在当前目录根下
- 用户在哪里运行 opencode，哪里就是项目根目录
